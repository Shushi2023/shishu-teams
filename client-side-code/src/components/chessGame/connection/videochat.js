...
function VideoChatApp(props) {
  ...
  function callPeer(id) {
    ...
    peer.on("signal", data => {
      socket.emit("callPlayer", { userToCall: id, signalData: data, from: props.mySocketId})
    })
  ...
  if (callAccepted) {
    ...
    mainView = (
      <div>
        <h1 style = {{color : "white"}}>{props.opponentUserName} is calling you</h1>
    ...
    mainView = (
      <div>
        <h1 style = {{color : "white"}}>Currently calling {props.opponentUserName}...</h1>
      </div>
    )
  } else {
    mainView = (
